...
class FileService:
    ...
    def extract_text_from_pdf(self, file_storage) -> str:
    ...
    def extract_text_from_csv(self, file_storage) -> str:
...
class AudioService:
    """Handles audio processing, STT, TTS, and language detection"""
    
    def __init__(self, api_key: str, translation_service: TranslationService):
        self.api_key = api_key
        self.translation_service = translation_service
        
    def speech_to_text(self, audio_base64: str, source_language: str) -> Optional[Dict]:
    ...
    def detect_language(self, audio_base64: str) -> Optional[str]:
...
class DocumentManager:
    ...
    def __init__(self, document_processor: DocumentProcessor, translation_service: TranslationService):
        self.document_processor = document_processor
        self.translation_service = translation_service
        self.processed_documents = {}
        self.document_cache = {}
        
    def process_and_store_document(self, file, chat_id: str, target_language: str) -> Dict:
        ...
        self.document_cache[chat_id] = temp_path
        
        try:
            # Process the document
            result = self.document_processor.process_document(temp_path)
            if not result["success"]:
                return {"success": False, "error": result.get("error", "Failed to process document")}
                
            key_info = self.document_processor.extract_key_information(result["content"])
            ...
            if target_language != "en-IN":
                ...
                translation_result = self.document_processor.translate_document_content(
                    result["content"], 
                    target_language
                )
            
            # Store processed document info for future chat messages
            self.processed_documents[chat_id] = {
    ...
    def translate_document(self, chat_id: str, target_language: str) -> Dict:
        """Translate a previously processed document"""
        if chat_id not in self.processed_documents:
            return {"success": False, "error": "No document found for this session"}
        
        doc_info = self.processed_documents[chat_id]
        ...
        try:
            ...
            if translation_result["success"]:
                ...
                self.processed_documents[chat_id]["translated_content"] = translation_result["translated_content"]
                self.processed_documents[chat_id]["language"] = target_language
                ...
                return {"success": False, "error": "Failed to translate document"}
                
        except Exception as e:
            logger.error(f"Error translating document: {str(e)}")
            return {"success": False, "error": f"Error translating document: {str(e)}"}
            
    def get_document_context(self, chat_id: str, user_language: str) -> str:
        """Get document context for a chat session"""
        if chat_id not in self.processed_documents:
            return ""
...
class FinanceApp:
    ...
    def __init__(self):
        ...
        CORS(self.app)
        self.app.secret_key = os.getenv('SECRET_KEY', "your_unique_secret_key")
        self.socketio = SocketIO(self.app, cors_allowed_origins="*")
        
        # API key
        self.api_key = os.getenv("SARVAM_API_KEY", "b7e1c4f0-4c19-4d34-8d2f-6aea1990bdbf")
        
        # Initialize services
        self.translation_service = TranslationService(self.api_key)
        self.audio_service = AudioService(self.api_key, self.translation_service)
        self.file_service = FileService()
        self.chat_history = ChatHistory()
        self.document_processor = DocumentProcessor(self.api_key)
        self.document_manager = DocumentManager(self.document_processor, self.translation_service)
    ...
    def _register_routes(self):
        ...
        @self.app.route('/upload-document', methods=['POST'])
        def upload_document():
            ...
            try:
                result = self.document_manager.process_and_store_document(file, chat_id, target_language)
                if result["success"]:
        ...
        # Translate document route
        @self.app.route('/translate-document', methods=['POST'])
        def translate_document():
            data = request.json
            chat_id = data.get('chat_id', 'default')
            target_language = data.get('target_language', 'en-IN')
            
            result = self.document_manager.translate_document(chat_id, target_language)
            
            if result["success"]:
                return jsonify(result)
            else:
                return jsonify(result), 400 if "error" in result else 500
                
        # Interest rates endpoint
        @self.app.route('/interest-rates', methods=['GET'])
        ...
        @self.app.route('/recent-queries', methods=['GET'])
        ...
        @self.app.route('/financial-tips', methods=['GET'])
        ...
        @self.app.route('/generate-summary', methods=['OPTIONS'])
        ...
        @self.app.route('/download-summary', methods=['OPTIONS'])
    ...
    def _register_socket_handlers(self):
        ...
        def handle_send_message(msg):
            try:
                ...
                self.chat_history.add_message(chat_id, user_message, True, user_language)
                
                # Get document context if available
                document_context = self.document_manager.get_document_context(chat_id, user_language)
                if document_context:
                ...
                if "msg" in result.get("res", {}):
                    self.chat_history.add_message(chat_id, result["res"]["msg"], False, user_language)
        ...
        @self.socketio.on('audio_message')
        def handle_audio_message(data):
            try:
                ...
                document_context = self.document_manager.get_document_context(session_id, current_language)
                ...
                if auto_detect:
                    detected_lang = self.audio_service.detect_language(data['audio'])
                    if detected_lang:
                ...
                stt_result = self.audio_service.speech_to_text(data['audio'], current_language)
                if not stt_result:
                ...
                self.chat_history.add_message(session_id, original_text, True, current_language)
                ...
                if needs_translation:
                    translated_text = self.translation_service.translate_text(original_text, current_language, "en-IN")
                ...
                if needs_translation:
                    ...
                    translated_response = self.translation_service.translate_text(english_response, "en-IN", current_language)
                    logger.info(f"Translation successful. Length: {len(translated_response)}")
                else:
                    translated_response = english_response
                
                # Save bot response to history
                self.chat_history.add_message(session_id, translated_response, False, current_language)
                
                # Generate audio in detected language
                logger.info(f"Generating audio in language: {current_language}")
                audio_data = self.audio_service.generate_audio(translated_response, current_language)
        ...
        @self.socketio.on('check_voice_support')
