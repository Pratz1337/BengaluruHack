...
extractionLLM = ChatGoogleGenerativeAI(
    ...
    timeout=None,
    max_retries=2,
    google_api_key="AIzaSyCdIOTcvJOgnGdlELK8lCz6ezwruz7sE9E"
)
...
def extract_college_info(content):
    ...
    - course: The course being inquired about (if mentioned and if not mentioned put ""). This is based only on the last message of the user and not the previous message. Should be only 1 course and not more
    - fees: The fee structure being inquired about, don't mention random parts from the conversation but only if the conversation contains actual fees mentioned (if mentioned and if not mentioned put ""). Should be based on only the course being enquired about and not for all courses. Make sure this is always fees for 1 course
    - cutoff: The cutoff information being inquired about (THIS HAS TO BE IN A JSON FORMAT BECAUSE IT HAS TO BE MADE INTO A TABLE) (if mentioned and if not mentioned put ""). ALWAYS KEEP IT IN THE FORM (branch name: (2024: (general: (opening_rank, closing_rank), OBC: (opening_rank, closing_rank), SC: (opening_rank, closing_rank)))) ONLY FOLLOW THIS FORMAT ALWAYS. THIS SHOULD DEPEND ON THE LAST MESSAGE IN THE ARRAY. IF CUTOFF IS NOT THE LAST MESSAGE RETURN EMPTY DICTIONARY. THIS SHOULD DEPEND ON THE LAST MESSAGE IN THE ARRAY AND NOT ON THE PREVIOUS MESSAGES. IF CUTOFF IS 0 JUST REMOVE FROM ARRAY
    ...
    - options: This should be a list of all the possible follow up questions that the user can ask based on the conversation flow and the last message in the array. Don't suggest random stuff and stick to the current conversation history provided .Limit yourself to 3 options and keep them as short as possible. Also keep it within the domain of (courses, fees, scholarship and college names, placements, exams, events).
    - dropdown_items: When the chatbot has mentioned multiple actions that can be performed or multiple names of colleges or courses then add all of them to this list. This list would be used as a drop down for choices. THIS SHOULD BE BASED ON THE LAST MESSAGE IN THE ARRAY. Make sure all the actions/options in the last messages are present in the array. NO ABBRIVATIONS OF NAMES ALLOWED
    - link: This is the link of the website of the college currently being talked about by the user. You can use the link mentioned in one of the messages in the array along with the college name
    ...
    THE LAST 2 ENTRIES IN THE ARRAY ARE THE MOST RECENT CONVERSATION AND SHOULD BE AN IMPORTANT FACTOR FOR DETERMINING THE ABOVE PARAMETERS
    """
...
class GoogleLLMService(LLMService):
    ...
    async def _process_context(self, context: OpenAILLMContext):
        ...
        try:
            ...
            send_messages = context.get_messages_for_logging()
            extracted = {"name": "", "show": "", "number_of_tickets": 0, "total_amount": 0}
            extracted = extract_college_info(send_messages)
            try:
                if len(send_messages) == 1:
                    pass
                elif send_messages[-1]["parts"][0].get("text") and send_messages[-2]["parts"][0].get("text"):
                    send_data = {
                    "info": extracted,
                    "res": {"msg": [send_messages[-2]["parts"][0]["text"],send_messages[-1]["parts"][0]["text"]], "toolCall": {}}
                }

                    res = requests.post(f"{os.environ["API_URL"] if os.environ["API_URL"] else "http://localhost:5000"}/voice/response", json=send_data)
            except:
                print("handle tool call here")
